...
template < class TProblemSolution > TNSGAIIC < TProblemSolution >::TNSGAIIC()
{
...
template < class TProblemSolution >
    ...
    int iNumberOfGenerations, double dMutationProbability, double dPopulationGeometricalFactor, bool bScalarize)
{
...
template < class TProblemSolution > void TNSGAIIC < TProblemSolution >::Run()
{
    vector < IntVector > iF;
    ...
    for (i = 0; i < this->m_iPopulationSize; i++)
    {
        ...
        // recombination                
        pOffspring =
            new TProblemSolution(*(TProblemSolution *) m_oPPopulation[iSolution1],
            *(TProblemSolution *) m_oPPopulation[iSolution2]);
    ...
    do
    {
            ...
            for (i = 0; i < NumberOfObjectives; i++)
            {
                ...
                for (j = 1; j < iFrontSize - 2; j++)
                {
                    ...
                    if (dDistance[iSolution] >= 0.0)
                    {
                        ...
                        d = m_oRPopulation[iSolution1]->ObjectiveValues[i] -
                            m_oRPopulation[iSolution2]->ObjectiveValues[i];

                        // optional scalarization 
                        if (this->m_bScalarize == true)
                        {
                            dDifference =
                                m_oRPopulation.ApproximateIdealPoint.ObjectiveValues[i] -
                                m_oRPopulation.ApproximateNadirPoint.ObjectiveValues[i];
                            if (dDifference != 0.0)
                            {
                                d = d / dDifference;
                            } else
        ...
        {
            iSolution1 =
                TMOTools::GetSolutionUsingBinaryTournamentNischedMin(m_oPPopulation, dParentFitness, dParentDistance);
            iSolution2 =
                TMOTools::GetSolutionUsingBinaryTournamentNischedMin(m_oPPopulation, dParentFitness, dParentDistance);
...
template < class TProblemSolution > void TDisplayNSGAIIC < TProblemSolution >::End()
{
...
template < class TProblemSolution > void TDisplayNSGAIIC < TProblemSolution >::NewNondominatedSolutionFound()
{
    cout << this->pNondominatedSet->iSetSize << '\t';
    int iobj;
    for (iobj = 0; iobj < NumberOfObjectives; iobj++)
...
template < class TProblemSolution > void TDisplayNSGAIIC < TProblemSolution >::NewGenerationFound()
{

...
template < class TProblemSolution > TNSGA < TProblemSolution >::TNSGA()
{
    this->m_iPopulationSize = 100;
    this->m_iNumberOfGenerations = 1000;
    this->m_dMutationProbability = 0.2;
    m_dNeighbourhoodDistance = 0.5;
    m_bScalarize = 0;
}

template < class TProblemSolution >
    void TNSGA < TProblemSolution >::SetParameters(int iPopulationSize,
    int iNumberOfGenerations, double dMutationProbability, double dNeighbourhoodDistance, bool bScalarize)
{
    ...
    m_dNeighbourhoodDistance = dNeighbourhoodDistance;
    m_bScalarize = bScalarize;
}

template < class TProblemSolution > void TNSGA < TProblemSolution >::Run()
{
    ...
    do
    {
        ...
        for (i = 0; i < this->m_iPopulationSize; i++)
        {
            ...
            pOffspring =
                new TProblemSolution(*(TProblemSolution *) m_oParentPopulation[iSolution1],
                *(TProblemSolution *) m_oParentPopulation[iSolution2]);
...
template < class TProblemSolution > void TDisplayNSGA < TProblemSolution >::End()
{
...
template < class TProblemSolution > void TDisplayNSGA < TProblemSolution >::NewNondominatedSolutionFound()
{
    cout << this->pNondominatedSet->iSetSize << '\t';
    int iobj;
    for (iobj = 0; iobj < NumberOfObjectives; iobj++)
...
template < class TProblemSolution > void TDisplayNSGA < TProblemSolution >::NewGenerationFound()
{

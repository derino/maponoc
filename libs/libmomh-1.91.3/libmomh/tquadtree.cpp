...
template < class TProblemSolution > TQuadTree < TProblemSolution >::TQuadTree():TNondominatedSet()
{
    iSetSize = 0;
    m_pQuadRoot = NULL;
    m_bQueueSort = true;
...
template < class TProblemSolution >
    ...
    TSolution * pSolution, int iListPosition, vector < bool > &bKSuccessor)
{
...
template < class TProblemSolution >
    bool TQuadTree < TProblemSolution >::VerifyRemoved(TSolution & oNewSolution, vector < int >&iReinsertIndices)
{
...
template < class TProblemSolution >
    bool TQuadTree < TProblemSolution >::DeleteNode(TQuadNodePtr & pCurrentNode, vector < int >&iReinsertIndices)
{
...
template < class TProblemSolution >
    ...
    TQuadNode & oQuadNode, bool & bIsDominated)
{
...
template < class TProblemSolution >
    bool TQuadTree < TProblemSolution >::FindDominatingInTree(TSolution & oNewSolution, bool & bIsDominated)
{
    bIsDominated = false;
    ...
    TQuadNodePtr & pCurrentNode, vector < int >&iReinsertIndices)
{
    ...
    if (iResult == _Dominated)
    {
        ...
        if (DeleteNode(pCurrentNode, iReinsertIndices) == false)
        {
    ...
    for (i = 0; i < oOffsprings.size(); i++)
    {
        if (FindRecursiveAndRemoveDominatedInTree(oNewSolution, oOffsprings[i], iReinsertIndices) == false)
        {
...
template < class TProblemSolution >
    bool TQuadTree < TProblemSolution >::AddRecursiveNewSolution(TQuadNodePtr & pCurrentNode, TSolution * pSolution)
{
    TQuadNode *pParentNode = NULL;
    TQuadNode *pQuadNode = NULL;
    vector < bool > bKSuccessor;
    ...
    {
        // go to subnode
        if (AddRecursiveNewSolution(pQuadNode, pSolution) == false)
        {
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::AddNewSolution(TSolution * pSolution)
{
    ...
    {
        if (AddRecursiveNewSolution(m_pQuadRoot, pSolution) == false)
        {
...
template < class TProblemSolution >
    bool TQuadTree < TProblemSolution >::AddRecursiveExistSolution(TQuadNodePtr & pCurrentNode, int iListPosition)
{
    TQuadNode *pParentNode = NULL;
    TQuadNode *pQuadNode = NULL;
    vector < bool > bKSuccessor;
    ...
    {
        // go to subnode
        if (AddRecursiveExistSolution(pQuadNode, iListPosition) == false)
        {
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::AddExistSolution(int iListPosition)
{
    // insert root if not exist
    if (m_pQuadRoot == NULL)
    {
    ...
    {
        if (AddRecursiveExistSolution(m_pQuadRoot, iListPosition) == false)
        {
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::Update(TSolution & Solution)
{
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::Update(TNondominatedSet & NondominatedSet)
{
    bool bAdded = false;
...
template < class TProblemSolution > ostream & TQuadTree < TProblemSolution >::Save(ostream & oStream, int iSpace = 0)
{
    char *pSpace = NULL;
 ...
 **/
template < class TProblemSolution >
    void TQuadTree < TProblemSolution >::QueueSortObjective(int iObjective, int iStartPos, int iEndPos,
    vector < int >&iIndices, vector < int >&iPositions)
{
...
template < class TProblemSolution >
    void TQuadTree < TProblemSolution >::QueueSortReinsert(int iStart, int iEnd, vector < IVector > &oIndicesMatrix,
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::QueueSort(vector < int >&iReinsertIndices)
{
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::QueueRandomly(vector < int >&iReinsertIndices)
{
    time_t oTime;
...
template < class TProblemSolution > bool TQuadTree < TProblemSolution >::CompactTree()
{
    vector < int >iReinsertIndices;
...
template < class TProblemSolution > void TQuadTree < TProblemSolution >::DeleteAll()
{
    // remove whole tree
    if (m_pQuadRoot != NULL)
    {
...
template < class TProblemSolution > void TQuadTree < TProblemSolution >::GetRandomSolution(TSolution * &pSolution)
{
    int iIndex = 0;
    int iTimeout = 10;
...
template < class TProblemSolution > void TQuadTree < TProblemSolution >::UpdateNadir(TSolution & Solution)
{
    int iObjective = 0;
    unsigned int i = 0;
    bool bFirst = true;
                ...
                if (Solution.ObjectiveValues[iObjective] == ApproximateNadirPoint.ObjectiveValues[iObjective])
                {
                    ...
                    for (i = 0; i < size(); i++)
                    {
                        ...
                        } else
                        {
                            ...
                            if (Objectives[iObjective].ObjectiveType == _Max)
                            {
                                if (ApproximateNadirPoint.ObjectiveValues[iObjective] >
                                    (*this)[i]->ObjectiveValues[iObjective])
                                {
                                    ApproximateNadirPoint.ObjectiveValues[iObjective] =
                                        (*this)[i]->ObjectiveValues[iObjective];
                                }
                            }
...
template < class TProblemSolution > void TQuadTree < TProblemSolution >::UpdateIdealAndNadir(TSolution & Solution)
{
    int iObjective = 0;

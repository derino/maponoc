...
#ifndef __HGABASE_CPP_
#define __HGABASE_CPP_
    ...
    double NewSolutionValue)
{
    ...
    if (VirtualPopulation.size() < TempPopulationSize)
        bAdd = true;
    else if (NewSolutionValue < VirtualPopulation[VirtualPopulation.size() - 1])
        bAdd = true;

    if (bAdd)
    {
        // If new solution is worse than currently worst add it at the end
        if (NewSolutionValue > VirtualPopulation[VirtualPopulation.size() - 1])
            VirtualPopulation.push_back(NewSolutionValue);

        // If new solution is better than currently best add it at the begining
        else if (NewSolutionValue < VirtualPopulation[0])
        ...
        else
        {
            ...
            while (!bPlaced)
            {
                ...
                } else
                {
                    ...
                    if (NewSolutionValue >= VirtualPopulation[Pos3])
                        Pos1 = Pos3;
                    else
                        Pos2 = Pos3;
                }
...
template < class TProblemSolution > void THGABase < TProblemSolution >::FindInitialPopulation()
{
    ...
    do
    {
        ...
        {
            if ((MainPopulation.size() > TempPopulationSize) && (MainPopulation.size() % TempPopulationSize == 0))
            {
                PopulationEvaluation = EvaluateInitialPopulation();
                bEnd = PopulationEvaluation <= 0;
            }
        } else
            bEnd = (MainPopulation.size() >= InitialPopulationSize) && (MainPopulation.size() >= TempPopulationSize);
    } while (!bEnd);
...
template < class TProblemSolution > double THGABase < TProblemSolution >::EvaluateInitialPopulation()
{
    double PopulationEvaluation = 0;

    // For all solutions
    unsigned int isol;
    for (isol = 0; isol < MainPopulation.size(); isol++)
    {
        ...
        unsigned int isol2;
        for (isol2 = iStartingSolution; isol2 < MainPopulation.size(); isol2++)
        {
            double SolutionValue =
                MainPopulation[isol2]->ScalarizingFunction(this->pNondominatedSet->ApproximateIdealPoint, WeightVector);
            if (isol2 != isol)
            {
        ...
        double Sum = 0;
        unsigned int i;
        for (i = 0; i < TempPopulationSize; i++)
        {
    ...
    bool THGABase < TProblemSolution >::UpdateTempPopulation(TPoint & ReferencePoint, TMOMHSolution & Solution)
{
    ...
    Solution.ScalarizingFunctionValue = Solution.ScalarizingFunction(ReferencePoint, WeightVector);
    ...
    if (TempPopulation.size() < TempPopulationSize)
        bAdd = true;
    else if (Solution.ScalarizingFunctionValue < TempPopulation[TempPopulation.size() - 1]->ScalarizingFunctionValue)
        bAdd = true;
    ...
    if (bAdd)
    {
        ...
        // If new solution is worse than currently worst add it at the end
        if (Solution.ScalarizingFunctionValue > TempPopulation[TempPopulation.size() - 1]->ScalarizingFunctionValue)
        {
            TempPopulation.push_back(&Solution);
            bAdded = true;
        }
        // If new solution is better than currently best add it at the begining
        else if (Solution.ScalarizingFunctionValue < TempPopulation[0]->ScalarizingFunctionValue)
        {
        ...
        else
        {
            ...
            if ((Solution.ScalarizingFunctionValue ==
                ...
                (Solution.ScalarizingFunctionValue == TempPopulation[0]->ScalarizingFunctionValue))
                bEqual = true;
            ...
            while (!bPlaced && !bEqual)
            {
                ...
                } else
                {
                    ...
                    int Pos3 = (Pos2 + Pos1) / 2;
                    if (Solution.ScalarizingFunctionValue == TempPopulation[Pos3]->ScalarizingFunctionValue)
                    {
                        bEqual = true;
                    } else
                    {
    ...
    TScalarizingFunctionType ScalarizingFunctionType, TWeightVector & WeightVector, bool bUpdateNondominatedSet)
{
    ...
    for (i = 0; i < PopulationSize; i++)
    {
        ...
        TProblemSolution *Solution =
            new TProblemSolution(ScalarizingFunctionType, ReferencePoint, WeightVector, TempNondominatedSet);
    ...
    do
    {
        ...
        TListSet < TProblemSolution > TempNondominatedSet;
        TProblemSolution *Solution =
            new TProblemSolution(*Parent1, *Parent2, ScalarizingFunctionType, ReferencePoint, WeightVector,
            TempNondominatedSet);

...
#ifndef __MOMSLS_CPP_
#define __MOMSLS_CPP_
    ...
    TScalarizingFunctionType ScalarizingFunctionType, TWeightVector & WeightVector, bool bUpdateNondominatedSet)
{
    ...
    for (i = 0; i < NumberOfIterations; i++)
    {
        ...
        else if (BestSolution.ScalarizingFunction(ReferencePoint) > Solution->ScalarizingFunction(ReferencePoint))
            BestSolution = *Solution;
...
template < class TProblemSolution > void TMOMSLS < TProblemSolution >::Run()
{
    // Callback method
    this->Start();
    ...
    } while (!this->bStop && ((this->bOnlineOfflineEvaluating) || (Iteration < NumberOfIterations)));

    // Callback method
    this->End();
}

template < class TProblemSolution > void TDisplayMOMSLS < TProblemSolution >::End()
{
...
template < class TProblemSolution > void TDisplayMOMSLS < TProblemSolution >::NewNondominatedSolutionFound()
{
    cout << this->pNondominatedSet->iSetSize << '\t';
    int iobj;
    for (iobj = 0; iobj < NumberOfObjectives; iobj++)

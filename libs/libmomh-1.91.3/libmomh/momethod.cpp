...
#ifndef __MOMETHOD_CPP_
#define __MOMETHOD_CPP_
...
template < class TProblemSolution > bool TMOMethod < TProblemSolution >::InitializeNondominatedSet(int iType)
{
    ...
    int WeightSpaceSamplingParameter)
{
    ...
    for (i = 0; i < WeightsSet.size(); i++)
    {
        ...
        {
            SingleObjectiveOptimization(pNondominatedSet->ApproximateIdealPoint, ScalarizingFunctionType, WeightsSet[i],
                true);
            cout << '.';
        }
    }
    ...
    TScalarizingFunctionType ScalarizingFunctionType, double MaximumEffectivenessIndex)
{
    ...
    for (iobj = 0; iobj < NumberOfObjectives; iobj++)
    {
        if (Objectives[iobj].bActive)
        {
            ...
            int iobj1;
            for (iobj1 = 0; iobj1 < NumberOfObjectives; iobj1++)
            {
    ...
    for (i = 0; i < NumberOfWeightVectors; i++)
    {
        EvaluationWeightsSet[i].Rescale(SolutionsSet.ApproximateIdealPoint, SolutionsSet.ApproximateNadirPoint);
    }
    ...
    for (i = 0; i < NumberOfWeightVectors; i++)
    {
        ...
        TPoint Point =
            SingleObjectiveOptimization(ReferencePoint, ScalarizingFunctionType, EvaluationWeightsSet[i], false);
    ...
    void TMOMethod < TProblemSolution >::NewSolutionGenerated(TProblemSolution & NewSolution)
{
    if (bOnlineOfflineEvaluating)
    {
        ...
        if (ElapsedTime > TimeUnit)
        {
            ...
            if (AverageOfflineScalarizingFunctionValue < AverageOnlineScalarizingFunctionValue)
                bStop = true;

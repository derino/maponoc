...
template < class TProblemSolution > TPSA < TProblemSolution >::TPSA()
{
...
template < class TProblemSolution > void TPSA < TProblemSolution >::FindInitialPopulation()
{
    int i;
    for (i = 0; i < GeneratingPopulationSize; i++)
    {
        ...
        GeneratingPopulationElement->WeightVector = GeneratingPopulationElement->GeneratingSolution->WeightVector;

        GeneratingPopulation.push_back(GeneratingPopulationElement);
    }
}

template < class TProblemSolution > void TPSA < TProblemSolution >::UpdateWeightVector(int isol)
{
    if (GeneratingPopulationSize > 1)
    {
        ...
        int isol2;
        for (isol2 = 0; isol2 < GeneratingPopulationSize; isol2++)
        {
            if (isol != isol2)
            {
                if (GeneratingPopulation[isol]->GeneratingSolution->Compare((TProblemSolution &) *
                        GeneratingPopulation[isol2]->GeneratingSolution) == _Nondominated)
                {
                    if (bFirstIter)
                    {
                        bFirstIter = false;
                        MinDistance =
                            GeneratingPopulation[isol]->GeneratingSolution->Distance((TProblemSolution &) *
                            GeneratingPopulation[isol2]->GeneratingSolution,
                            this->pNondominatedSet->ApproximateIdealPoint,
    ...
    GeneratingPopulation[isol]->GeneratingSolution->WeightVector = GeneratingPopulation[isol]->WeightVector;
    GeneratingPopulation[isol]->GeneratingSolution->WeightVector.Rescale(this->pNondominatedSet->ApproximateIdealPoint,
        this->pNondominatedSet->ApproximateNadirPoint);
}

template < class TProblemSolution > void TPSA < TProblemSolution >::Run()
{
...
template < class TProblemSolution > void TDisplayPSA < TProblemSolution >::End()
{
...
template < class TProblemSolution > void TDisplayPSA < TProblemSolution >::NewNondominatedSolutionFound()
{
}

template < class TProblemSolution > void TDisplayPSA < TProblemSolution >::TemperatureDecreased()
{

...
int TMOTools::GetRandomSolutionUsingFitnessMax(vector < TSolution * >&oSolutions, vector < double >&dFitness,
    double dSumOfFitness)
{
    ...
    i = 0;
    while ((i < (oSolutions.size() - 1)) && (dTempSum < dRandomSumOfFitnesses))
    {
        dTempSum += dFitness[i];
        i += 1;
    }
...
int TMOTools::GetSolutionUsingBinaryTournamentNischedMin(vector < TSolution * >&oSolutions, vector < double >&dFitness,
...
bool TMOTools::ConstrainedFastNondominatedSort(TSolutionsSet & oSolutions, vector < IntVector > &iF)
{
    ...
    for (i = 0; i < iPopulationSize; i++)
    {
        for (j = 0; j < iPopulationSize; j++)
        {
	  /** OD: when they are same, sometimes it happens that they are compared as not equal (i.e. dominating). Probably due to machine precision errors. It is better not to compare them at all.*/
	  if(i == j)
	    continue;
	    ...
	    // cout << "p(" << i << "): " << pP->ObjectiveValues[0] << "\t" << pP->ObjectiveValues[1] 
	    // 	 << "\t" << pP->TotalConstraintsViolation()
	    // 	 << " vs. q(" << j << "): " << pQ->ObjectiveValues[0] << "\t" << pQ->ObjectiveValues[1]
	    // 	 << "\t" << pQ->TotalConstraintsViolation()
	    // 	 << endl;
            ...
            if (eCompareResult == _Dominating)
            {
	      //	      cout << "p(" << i << ") dominates q(" << j << ")" << endl;
	      iS[i].push_back(j);
            }

            if (eCompareResult == _Dominated)
            {
	      //	      cout << "p(" << i << ") is dominated by q(" << j << ")" << endl;
	      iN[i] += 1;
            }
	    /** OD */
	    // if (eCompareResult == _Equal && i != j )
	    //   {
	    // 	cout << "EQUALLLLLLLLLL " << i << "\t" << j << endl;
	    //   }
        }
        ...
        if (iN[i] == 0)
        {
	  // cout << "p(" << i << ") is added to iF[0]" << endl;
	  iF[0].push_back(i);
	  bA[i] = true;
        }
    }
    ...
    while (iF[i - 1].size() > 0)
    {
        ...
        {
            iSolutionP = iF[i - 1][j];
	    //if(i>=526) cout << "iSolutionP:" << iSolutionP << endl;
            // check dominated solutions by solution from current front
            for (k = 0; k < iS[iSolutionP].size(); k++)
            {
                iSolutionQ = iS[iSolutionP][k];
                iN[iSolutionQ] -= 1;
		//if(i>=526) cout << "iN[" << iSolutionQ << "]" << iN[iSolutionQ] << endl;
                if ((iN[iSolutionQ] == 0) && (bA[iSolutionQ] == false))
                {
                    iF[i].push_back(iSolutionQ);
                    bA[iSolutionQ] = true;
	...
	//	cout << "iF[i - 1].size(): " << iF[i - 1].size() << endl;
    }
    ...
    //   {
    // 	//TSolution* ms = oSolutions[ iF[i][j] ];
    // 	TDPAMConstrainedSolution* ms = (TDPAMConstrainedSolution *) oSolutions[ iF[i][j] ];
    // 	cout << "Front " << i << " p(" << iF[i][j] << "): " << ms->ObjectiveValues[0] << " " << ms->ObjectiveValues[1] << "\t" << ms->TotalConstraintsViolation() << endl;
    //   }
    
    return true;
}

void TMOTools::RecursiveQuickSortSolutionsMax(int iStartPos, int iEndPos, TSolutionsSet & oSolutionSet,
...
void TMOTools::RecursiveQuickSortObjectiveMax(int iObjective, int iStartPos, int iEndPos, TSolutionsSet & oSolutions,
    IntVector & iIndices)
{
    int iCurrentStartPos = 0;
    int iCurrentEndPos = 0;
...
bool TMOTools::QuickSortByObjectivesMax(TSolutionsSet & oSolutions, IntVector & iSelectedIndices,
...
void TMOTools::RecursiveQuickSortByValueMax(int iStartPos, int iEndPos, TSolutionsSet & oSolutionSet,

...
bool TBTree::AddBNode(vector < bool > &oValue, TQuadNode * pQuadNode, TQuadNode ** pPreviousNode)
{
    ...
    if ((oValue.size() != NumberOfObjectives) || (pQuadNode == NULL) || (pPreviousNode == NULL))
    {
...
bool TBTree::RemoveBNode(vector < bool > &oValue, TQuadNode ** pPreviousNode)
{
    ...
    if ((oValue.size() != NumberOfObjectives) || (pPreviousNode == NULL))
    {
    ...
    *pPreviousNode = NULL;
    ...
    {
        pNode = oNodeVector[i];
        if ((pNode->m_pOneOffspring == NULL) && (pNode->m_pZeroOffspring == NULL) && (pNode->m_pQuadNode == NULL))
        {
...
bool TBTree::GetRecursiveGreater(TBNode * pNode, unsigned int iLevel, bool bEqual, vector < bool > &oValue,
    vector < TQuadNode * >&oNodes)
{
    if (((pNode->m_pOneOffspring != NULL) || (pNode->m_pZeroOffspring != NULL)) && (pNode->m_pQuadNode != NULL))
    {
    ...
    if ((pNode->m_pZeroOffspring == NULL) && (pNode->m_pOneOffspring == NULL) && (pNode->m_pQuadNode != NULL)   /*&&
                                                                                                                   (bEqual == false) */ )
    {
...
bool TBTree::GetRecursiveLesser(TBNode * pNode, unsigned int iLevel, bool bEqual, vector < bool > &oValue,
    vector < TQuadNode * >&oNodes)
{
    if (((pNode->m_pOneOffspring != NULL) || (pNode->m_pZeroOffspring != NULL)) && (pNode->m_pQuadNode != NULL))
    {
